...
export default function GraderDashboard() {
  ...
  useEffect(() => {
    ...
    setEmail(mail);
    axios
      .get(`${process.env.NEXT_PUBLIC_BACKEND_URL}become/joinedGov/grader/${add}`)
      .then((res) => setGovs(res.data || []))
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);

  const displayName = email.split('@')[0] || 'Grader';
      ...
      <div className="relative rounded-2xl overflow-hidden p-6 text-white w-full" style={{ background: 'linear-gradient(135deg, #3E68FC 0%, #5b51f5 100%)' }}>
        ...
        <div className="relative flex items-center gap-5">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs uppercase tracking-widest font-medium mb-0.5" style={{ color: 'rgba(255,255,255,0.6)' }}>Grader</p>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-3" style={{ background: 'rgba(62,104,252,0.06)' }}>
              <ClipboardList size={24} style={{ color: 'rgba(62,104,252,0.3)' }} />
            </div>
            <p className="text-sm font-medium text-gray-500">No governances joined yet</p>
            <p className="text-xs text-gray-400 mt-1">You'll be able to enter marks once you're added to a governance</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {govs.map((g, i) => {
              ...
              return (
                <Link
                  key={i}
                  href={`/user/grader/marksDatabase/${g.governAdd}/${safeEncode(g.gName)}/${safeEncode(g.cName)}`}
                ...
                >
                  ...
                  <div className="p-5 flex flex-col gap-4 flex-1">
                    ...
                    <div className="pt-3 flex items-center justify-between" style={{ borderTop: '1px solid #f3f4f6' }}>
                      ...
                      <span className="text-xs font-semibold shrink-0" style={{ color: accent }}>Enter marks →</span>
                    </div>
                  </div>
                </Link>
              );
