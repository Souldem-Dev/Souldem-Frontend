...
let params = useParams()
  ...
  url = process.env.NEXT_PUBLIC_BACKEND_URL+"become/"+"becomeGrader"
    axios.post(url,{contractAdd:params.govAdd,memAdd:params.pKey,secretKey_1:formData.secret1,secretKey_2:formData.secret2,role:params.role,uniqueId:params.uniqueId,signature:params.sig,gName:params.govName,cName:params.univName,signerAdd:params.signer}).then(res=>{
      console.log(res)
    }).catch(err=>{
      console.log(err)
    })
  ...
  return (
    <div className="flex items-center justify-center  m-auto p-32">
      ...
      />
      <form autoComplete={false} className="flex w-full flex-col items-center bg-white  h-80 gap-y-4 p-4  px-32 relative  border-b-8 border-indigo-500 rounded-xl">
        {' '}
