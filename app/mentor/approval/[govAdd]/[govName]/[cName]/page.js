...
const Page = () => {
  ...
  let formik = useFormik({
    ...
    studentsEmail:Yup.string().email().required("This Field is required").strict().trim("no spaces allowed"),
    currentSemester:Yup.number().required("this field is required"),
    receiptNumber:Yup.string().required("this field is required").strict().trim("no spaces allowed")
    }),
    onSubmit:(resp)=>{
      let domain ={
        name: params.govName,
  ...
  return (
    <div className="w-full bg-white drop-shadow-md h-full py-8 px-4 md:p-20 flex flex-col gap-y-8 rounded-xl mx-20  my-20 md:my-auto">
      ...
      <form className="w-11/12 bg-white drop-shadow-md h-full py-8 px-4 md:p-20 my-8 flex flex-col gap-y-12 rounded-xl" onSubmit={formik.handleSubmit}>
        <div className="flex flex-col md:flex-row w-full gap-y-2">
          ...
          <div className="md:w-1/2">
            ...
            />
                      {formik.errors.studentsEmail?<p className='text-red-600'>{formik.errors.studentsEmail}</p>:null}
        ...
        <div className="flex flex-col md:flex-row w-full gap-y-2">
          ...
          <div className="md:w-1/2">
            <input
              ...
              onChange={formik.handleChange}
            />
             {formik.errors.currentSemester?<p className='text-red-600'>{formik.errors.currentSemester}</p>:null}
        ...
        <div className="flex flex-col md:flex-row w-full gap-y-2">
          ...
          <div className="md:w-1/2">
            <input
              ...
              onChange={formik.handleChange}
            />
            {formik.errors.receiptNumber?<p className='text-red-600'>{formik.errors.receiptNumber}</p>:null}
          </div>
        </div>
        <button className="px-4 py-2 rounded-md bg-blue text-white hover:border-2 hover:border-blue" type="submit">Approve</button>
      </form>
    </div>
  );
};
